...
class qDisCoCircIsIn():
    ...
    def __init__(self, train_circuits, valid_circuits, ansatz, wire_dim):
        ...
        self.text_model = PennyLaneModel.from_diagrams(self.train_circuits+self.valid_circuits, backend_config={"backend": "default.qubit", "normalize": False, "probabilities": False})
        self.text_model.initialise_weights()

        self.is_in_circ, self.is_in_circ_swapped = self.get_is_in_circs()
        self.is_in_model = PennyLaneModel.from_diagrams([self.is_in_circ, self.is_in_circ_swapped], backend_config={"backend": "default.qubit", "normalize": False, "probabilities": False})
    ...
    def forward(self, text_circuit, quesans):
        ...
        is_in_swapped_state = self.is_in_model([self.is_in_circ_swapped]).squeeze(0).flatten()
        ...
        for j in post_select_indices[::2]:
            ...
            for i in posel_ids[::-1]:
                sliced_text_state = sliced_text_state.select(i,0)
    ...
    def get_is_in_circs(self):
        ...
        is_in_circ = Ket(*[0 for i in range(2*self.wire_dim)]) >> self.ansatz(is_in_diag)
